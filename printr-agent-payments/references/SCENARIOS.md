...
| Payment succeeded but `/api/pay/verify` always returns unpaid                                 | Treasury ATA mismatch — you're looking for a USDC transfer to the wrong ATA         | Confirm that `TREASURY_RECEIVER_PUBKEY` matches the wallet whose USDC ATA the client tx targets. The verifier derives the treasury's USDC ATA lazily on first call — if `TREASURY_RECEIVER_PUBKEY` is wrong, the lazy-derived ATA will be wrong too. Check on Solana Explorer → Treasury wallet → Tokens tab. |
