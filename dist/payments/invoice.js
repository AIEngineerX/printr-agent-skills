...
import { createTransferCheckedInstruction, getAssociatedTokenAddress, TOKEN_PROGRAM_ID as SPL_TOKEN_PROGRAM_ID, } from '@solana/spl-token';
import { DECIMALS, MEMO_PROGRAM_ID, SUPPORTED_MINTS, USDC_MINT, WSOL_MINT, mintToCurrency, } from './constants.js';
/** Inlined to avoid depending on @solana/spl-memo for a 10-line helper. */
export function createMemoInstruction(memo, signers = []) {
...
export function generateInvoiceParams(opts) {
    if (!(opts.currency in SUPPORTED_MINTS)) {
        throw new Error(`unknown currency ${String(opts.currency)} — must be one of ${Object.keys(SUPPORTED_MINTS).join(', ')}`);
    }
    ...
    const memo = crypto.randomBytes(8).readBigUInt64BE() & 0x7fffffffffffffffn;
...
export async function buildPaymentTransaction(connection, params) {
    const currency = mintToCurrency(params.currency_mint);
    if (!currency) {
        throw new Error(`unsupported currency_mint ${params.currency_mint} — must be ${WSOL_MINT} (SOL) or ${USDC_MINT} (USDC)`);
    }
    ...
    }), ComputeBudgetProgram.setComputeUnitLimit({ units: 100_000 }), createMemoInstruction(params.memo.toString(), [user]));
    ...
    if (currency === 'SOL') {
        ...
        tx.add(createTransferCheckedInstruction(sourceAta, currencyMint, destAta, user, params.amount_smallest_unit, DECIMALS[currency], [], SPL_TOKEN_PROGRAM_ID));
    }
