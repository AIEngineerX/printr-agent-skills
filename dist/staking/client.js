...
export async function claimRewards(args, options) {
    ...
    const listResp = await listPositionsWithRewards({ owner: args.owner.publicKey, limit: 100 }, options);
    const matchedPositions = listResp.positions.filter((p) => args.positionIds.includes(p.info.position));
